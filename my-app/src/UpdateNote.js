...
function UpdateNote() {
    ...
    const NoteData = location.state.data.notes;
    ...
    const handlePre = ()=> { 
        ...
        ExistingNotes[index] = AllStates
        
        localStorage.setItem("NoteAppData", JSON.stringify(ExistingNotes));
    ...
    useEffect(()=>{
        if(location.state){
            ...
            setArchived(location.state?.data.archived || [])
